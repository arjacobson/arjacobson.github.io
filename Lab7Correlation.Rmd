...
edu <- c(12,13,18,16,17)
df <- data.frame(obs,income,edu)
x <- cor.test(df$income,df$edu,method = "pearson")
print(x$estimate)
...
edu <- c(12,13,18,16,17)
df <- data.frame(obs,income,edu)
cor(df$income, df$edu, method="spearman")
```
...
edu <- c(12,13,18,16,17)
df <- data.frame(obs,income,edu)
x <- cor.test(df$income, df$edu, method="spearman")
x
...
names(df) <- c("Year","Median income","Number of races won by leading jockey")
kable(df)
x <- cor.test(df$`Median income`,df$`Number of races won by leading jockey`,method = "pearson")
x 
...
Because the critical value is greater than the t-value, we cannot reject the null hypothesis that the true correlation coefficient is equal to zero. 
...
Therefore the correlation is equal to zero. 
...
Therefore the correlation coefficient is equal to zero. 
...
### Solution
```{r}
df <- read.csv(file="/Users/amberjacobson/Documents/Undergrad/Graduate/533 Statistics/Milwaukee_Sales.csv", header=TRUE, sep=",")
...
There is no relationship between number of bedrooms and lot size. 
...
### Solution
```{r}
df <- read.csv(file="/Users/amberjacobson/Documents/Undergrad/Graduate/533 Statistics/UK_Housing.csv", header=TRUE, sep=",")
...
This means that there is a large effect or rather significant relationship between floor area and bedrooms. 
...
This solution provides the same correlation coefficient as was calculated in the previous question. 
...
I reached the same correlation coeffecient with both equations. 
